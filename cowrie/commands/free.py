...
class command_free(HoneyPotCommand):
    ...
    def do_free(self, fmt='bytes'):
        ...
        if fmt=='bytes':
            ...
            Mem:       """+str(round(MEM_TOTAL / 1048576, 2 ))+'G    '+str(round((MEM_USED + var_mem)/1048576, 2))+'G     '+str(int(round((MEM_TOTAL - MEM_USED - var_mem)/1024)))+'M          0     '+str(int(round((MEM_BUFF + var_mem_buf)/1024)))+'M    '+str(round((MEM_CACHE + var_mem_cache)/1048576, 2))+"""G
            -/+ buffers/cache:    """+str(round((BUFF + var_buf)/1048576, 2))+'G    '+str(round((CACHE + var_cache)/1048576, 2))+"""G
            Swap:      """+str(round(SWP_TOTAL / 1048576, 2))+'G     '+str(int(round((SWP_USED + var_swp)/1024)))+'M    '+str(round((SWP_TOTAL - SWP_USED - var_swp)/1048576, 2))+'G'
